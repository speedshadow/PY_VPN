...
def dashboard(request):
    ...
    bots_counts = {k: 0 for k in bots_map}
    for bot_label, substrings in bots_map.items():
        regex = "|".join(substrings)
        bots_counts[bot_label] = Analytics.objects.filter(
            is_bot=True, timestamp__gte=seven_days_ago, user_agent__iregex=regex
        ).count()
